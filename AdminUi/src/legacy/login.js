...
try{const res=await fetch('/admin/api/session',{cache:'no-store'});if(!res.ok)throw Error('无法获取登录状态');const s=await res.json();if(s.authorized){location.replace(s.serverReady?next:'/admin-ui/database-setup.html');return}if(!s.initialized&&!s.databaseConfigured){if(s.loopback)location.replace('/admin-ui/database-setup.html');else $('hint').textContent='首次初始化请在服务器本机打开后台，先选择玩家数据存储。';return}setup=s.initialized===false;$('confirm-field').hidden=!setup;$('confirm-field').style.display=setup?'grid':'none';$('confirm').disabled=!setup;$('confirm').required=setup;
