...
<main class="page">
  ...
  <section class="card"><h2>对局历史保留</h2><p class="muted">清理任务每周运行一次，仅删除已结束对局；进行中的对局不会被清理。可按数量或结束时间保留，设置时间均为 UTC。</p>
    <form id="match-retention-form"><div class="form-grid">
      ...
      <label class="field"><span>保留天数</span><input id="retention-days" type="number" min="1" max="3650" required><small>时间模式生效；删除超过此天数的已结束对局。</small></label>
      <label class="field"><span>保留最近对局数量</span><input id="retention-count" type="number" min="1" max="1000000" required><small>数量模式生效；超过数量的较旧对局将被删除。</small></label>
      <label class="field"><span>每周清理星期（UTC）</span><select id="cleanup-day"><option value="0">星期日</option><option value="1">星期一</option><option value="2">星期二</option><option value="3">星期三</option><option value="4">星期四</option><option value="5">星期五</option><option value="6">星期六</option></select></label>
...
onMounted(async () => {
  ...
  await import('../legacy/system-settings.js');
});
</script>
